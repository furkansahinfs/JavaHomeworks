...
public class TransactionQueueList<T>  {
	...
	{
	 // To return specific TransactionQueue , code moves on one by one until given date equals to pos's date.
	...
	{
		//code moves on one by one until given date equals to TQ's date, after that I set the new TQ.
	...
	public String maxWorkload() {
		// To find maxWorkLoad, I create TQ max which keeps TQ contains max waiting time. Code moves on TQ one by one and compare the waiting times.
		TransactionQueue pos = head;
	...
	public void display()
	{
		// To display the TransactionQueuesList, code move on one by one on transactionQueue and print them.
