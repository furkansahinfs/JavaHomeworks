...
public class TransactionQueue<T> {
	...
	public void Time() {
		...
		Transaction positionNext = position.getNextTransaction(); // I create a transaction and equal it position's
																	// next.
		Customer positionData = (Customer) position.getData(); // I create a customer which contains position
																// transaction's data.
		Customer positionNextData = (Customer) positionNext.getData(); // I create a customer which contains position's
																		// next transaction's data.
		...
		for (int i = 0; i < queueLength - 1; i++) {
			...
			positionNext.setWaiting(positionData.getOccupation() + position.getWaiting()); // I set position next's
																							// waiting time with adding
																							// position's occupation
																							// time to position's
																							// waiting time
			position = position.getNextTransaction(); // I push forward position to next transaction.
			positionNext = positionNext.getNextTransaction(); // I push forward positionnext to position's next
																// transaction's next transaction.
		}

	}

	public void Sort() {
		// I sort transactionQueue according to priority of transactions with bubble
		// sort.
		for (int i = 0; i < queueLength - 1; i++) {

			Transaction position = head; // I create a transaction and equal it head firstly.
			Transaction positionNext = head.getNextTransaction(); // I create a transaction and equal it position's
																	// next.
			Customer positionData = (Customer) position.getData(); // I create a customer which contains position
																	// transaction's data.
			Customer positionNextData = (Customer) positionNext.getData(); // I create a customer which contains
																			// position's next transaction's data.

			for (int j = 0; j < (queueLength - i - 1); j++) {
				positionData = (Customer) position.getData(); // I update position's data
				positionNextData = (Customer) positionNext.getData(); // I update position next's data.
				if (positionData.getPriority() > positionNextData.getPriority()) // I compare priority of position and
																					// position's next through
																					// GetPriorty().
				{
	...
	public void display() {
		// To display the TransactionQueue, code move on one by one on transactions and
		// print their infos.
		Sort();
	...
	public String getDate() {
		// To get date of transactionQueue, I take date of transactionQueue through
		// head's data.
	...
	public String toString() {
		...
		// for example 01.05.2019 COUNTER <-
		// 101|CORPORATE|5|0<-102|INDIVIDUAL|4|5<-100|NON-REGISTERED|3|9<-103|NON-REGISTERED|2|12
		String TQ = "";
		Transaction temp = head;
		Customer tempData = (Customer) temp.getData();
		TQ = TQ + tempData.date;
		...
		for (int i = 0; i < queueLength; i++) {
			...
			TQ = (TQ + tempData.getId() + "|" + tempData.getType() + "|" + tempData.getOccupation() + "|"
					+ temp.getWaiting() + "<-");
			temp = temp.getNextTransaction();
		}
		return TQ;
