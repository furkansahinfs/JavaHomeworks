...
public class StorageArea {
	...
	/**
	 * We control that there exists enough space in the area if space is okey, then
	 * we add the item to the area and set the litre of area.
	 */
	...
	/**
	 * If there exists enough litre of item in the area, we remove the item. If
	 * litre of first item in area > given item's litre : set the litre of first
	 * item = litre of first item - given item's litre
	 * 
	 * If litre of first item in area < given item's litre : remove the first item
	 * in area set the given item's litre = given item's litre - litre of first item
	...
	public boolean remove(ICountable eleman) {
		...
		if (sayi <= litre) {
			...
			System.out.println(
					"Storage areada verilen türde bu kadar eleman yok. Tür : " + eleman.getClass().getSimpleName());
			result = false;
		}
