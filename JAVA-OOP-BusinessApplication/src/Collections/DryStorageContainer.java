...
public class DryStorageContainer implements IContainer {
	...
	/**
	 * If the given item is not countable, throws ItemNotCountableException If it is
	 * countable control that there exists enough space in the container then if
	 * container is not empty and given item is same with item in the container,
	 * invoke the addControl method to add. if container is empty, invoke the
	 * addControl method to add.
	 */
	 ...
	 * According to type we add the item to the container and set the litre of
	 * container.
	 */
	...
	public <T> boolean addControl(T obje) {
		...
		case "Yogurt":
			...
			break;
		default:
			throw new IllegalArgumentException("Unexpected item is tried to be given to the dry storage container: "
	 ...
	 * According to type we remove the item to the container and set the litre of
	 * container. If given item is unexpected type, we throw an
	...
	/**
	 * Firstly we analyze the type of the item (BoxedMilk, chocolate, yogurt) and
	 * create a new item which its litre is equal to litre of item in the container.
	 * After that we try to transfer (add to facitily) the item. If transfer is
	 * succesfull we invoke the remove method of container. If given item is
	 * unexpected type, we throw an IllegalArgumentException
	 */
