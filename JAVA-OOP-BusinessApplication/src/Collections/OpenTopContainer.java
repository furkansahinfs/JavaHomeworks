...
public class OpenTopContainer implements IContainer {
	...
	private int pozisyon = 2; // Pozisyon 1: warehouse, 2: factory, 3: distribution center. It holds the
	...
	/**
	 * If the given item is not uncountable, throws ItemNotUncountableException If
	 * it is uncountable control that there exists enough space in the container
	 * then if container is not empty and given item is same with item in the
	 * container, invoke the addControl method to add. if container is empty, invoke
	 * the addControl method to add.
	 */
	...
	/**
	 * Firstly we analyse the type of the item (yeast or cacao) According to type we
	 * add the item to the container and set the litre of container.
	 */
	...
	/**
	 * Firstly we analyse the type of the item (yeast or cacao) According to type we
	 * remove the item to the container and set the litre of container. If given
	 * item is unexpected type, we throw an IllegalArgumentException
	 */
	public <T> boolean remove(T obje) {
		...
		default:
			throw new IllegalArgumentException("Unexpected item is tried to be given to the open top container: "
					+ obje.getClass().getSimpleName());
	...
	/**
	 * Firstly we analyze the type of the item (yeast or cacao) and create a new
	 * item which its litre is equal to litre of item in the container. After that
	 * we try to transfer (add to facitily) the item. If transfer is succesfull we
	 * invoke the remove method of container. If given item is unexpected type, we
