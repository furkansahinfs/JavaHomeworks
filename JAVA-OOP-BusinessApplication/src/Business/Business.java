...
public class Business {
	...
	/**
	 * If given item is uncountable and there exists enough money then it invokes
	 * the buy method of warehouse. According to situation buying process is
	 * executed and money is fixed.
	 */
	...
	/**
	 * It invokes the sell method of distribution center. According to situation
	 * seling process is executed and money is fixed.
	 */
	...
	/**
	 * It invokes the producing method of warehouse. According to situation
	 * producing process is executed.
	 */
	...
	/**
	 * According to type of given item we calculate the cost of the given item
	 * according to its litre.
	 */
	...
	/**
	 * According to type of given item it invokes the transfer method of warehouse
	 * or factory. If item is milk or cream it invokes the warehouse transferTo
	 * method with tank container If item is cacao or yeast it invokes the warehouse
	 * transferTo method with open top container If item is product it invokes the
	 * factory transferTo method with dry storage container And according to
	 * container position cost is calculated and container position is set.
	 */
	public <T> void transfer(T eleman) throws Exception {
		if (eleman instanceof IUncountable) {
			 ...
			 * If not we firstly bring the dry container to the factory and send it to the
			 * distribution with item. During these operations, each cost of set of position
			 * is calculated and money is fixed.
			 */
	...
	/*
	 * If tank container is at the factory, we firstly bring the tank container to
	 * the warehouse If tank container is at the warehouse, we send it factory with
	 * item. During these operations, each cost of set of position is calculated and
	 * money is fixed.
	 */
	...
	/*
	 * If open top container is at the factory, we firstly bring the tank container
	 * to the warehouse If open top container is at the warehouse, we send it
	 * factory with item. During these operations, each cost of set of position is
	 * calculated and money is fixed.
	 */
	...
	public void display() {
		...
		System.out.println("Factory BoxedMilkArea: " + fac.getStorageArea().getSa_BoxedMilk().getLitre()
				+ " Litre --- BoxedMilk count is : " + fac.getStorageArea().getSa_BoxedMilk().getLitre());
		System.out.println("Factory ChocolateArea: " + fac.getStorageArea().getSa_Chocolate().getLitre()
				+ " Litre --- Chocolate count is : " + fac.getStorageArea().getSa_Chocolate().getLitre() * 2);
		System.out.println("Factory YogurtArea: " + fac.getStorageArea().getSa_Yogurt().getLitre()
				+ "Litre --- Yogurt count is : " + fac.getStorageArea().getSa_Yogurt().getLitre() * 0.5);
		System.out.println("------------------------------------");
		System.out.println("Distribution BoxedMilkArea: " + dis.getStorageArea().getSa_BoxedMilk().getLitre()
				+ " Litre --- BoxedMilk count is : " + dis.getStorageArea().getSa_BoxedMilk().getLitre());
		System.out.println("Distribution ChocolateArea: " + dis.getStorageArea().getSa_Chocolate().getLitre()
				+ " Litre --- Chocolate count is : " + dis.getStorageArea().getSa_Chocolate().getLitre() * 2);
		System.out.println("Distribution YogurtArea: " + dis.getStorageArea().getSa_Yogurt().getLitre()
				+ "Litre --- Yogurt count is : " + dis.getStorageArea().getSa_Yogurt().getLitre() * 0.5);
		System.out.println("------------------------------------");
