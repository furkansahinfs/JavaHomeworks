...
public class Warehouse extends Facilities {
	...
	/**
	 * We invoke the addStorageTanks method of StorageTanks class then StorageTanks
	 * class analyze that what is the given item's storageTank(milkTank or creamTank
	 * or cacaoTank or yeastTank) and try to add the item to the correct tank.
	 */
	...
	/**
	 * We invoke the removeStorageTanks method of StorageTanks class then
	 * StorageTanks class analyze that what is the given item's storageTank(milkTank
	 * or creamTank or cacaoTank or yeastTank) and try to remove the item from the
	 * correct tank.
	 */
	...
	/**
	 * If storage tank capacity of warehouse is enough for the given object,
	 * business buy ingredients.
	 */
	...
	/**
	 * If the given item is one of the milk,cacao,cream,yeast then we control that
	 * if the litre of warehouse storage tank of given item (milkTank or creamTank
	 * or cacaoTank or yeastTank) >= container capacity if it is yes : then we
	 * control that the litre of factory storage tank of given item (milkTank or
	 * creamTank or cacaoTank or yeastTank) is enough to transfer we transfer the
	 * fixed litre of item that container can take ( container ne kadar alabiliyorsa
	 * o kadar gönderiyoruz ).
	 * 
	 * if it is no : then we control that if the litre of warehouse storage tank of
	 * given item (milkTank or creamTank or cacaoTank or yeastTank) < container
	 * capacity then we control that the litre of factory storage tank of given item
	 * (milkTank or creamTank or cacaoTank or yeastTank) is enough to transfer we
	 * transfer the fixed litre of item that storage tank of given item(milkTank or
	 * creamTank or cacaoTank or yeastTank) includes. ( warehouse storage tanktaki
	 * (milkTank or creamTank or cacaoTank or yeastTank) litre kadar gönderiyoruz ).
	 * 
	 */
		...
		case "Milk":
			if (storageTanks.getMilkTank().getLitre() > 0) {
				if (storageTanks.getMilkTank().getLitre() >= ((TankContainer) container).getCapacity()) {
					...
					if ((((Factory) faci).getStorageTanks().getMilkTank().getLitre())
							+ temp.getLitre() <= ((Factory) faci).getStorageTanks().getMilkTank().getCapacity()) {
		...
		case "Cacao":
			if (storageTanks.getCacaoTank().getLitre() > 0) {
				if (storageTanks.getCacaoTank().getLitre() >= ((OpenTopContainer) container).getCapacity()) {
					...
					if ((((Factory) faci).getStorageTanks().getCacaoTank().getLitre())
							+ temp.getLitre() <= ((Factory) faci).getStorageTanks().getCacaoTank().getCapacity()) {
						result = transferControl(temp, faci, container);
					}
				}
		...
		case "Cream":
			if (storageTanks.getCreamTank().getLitre() > 0) {
				if (storageTanks.getCreamTank().getLitre() >= ((TankContainer) container).getCapacity()) {
					...
					if ((((Factory) faci).getStorageTanks().getCreamTank().getLitre())
							+ temp.getLitre() <= ((Factory) faci).getStorageTanks().getCreamTank().getCapacity()) {
						result = transferControl(temp, faci, container);
					}
				}
		...
		case "Yeast":
			if (storageTanks.getYeastTank().getLitre() > 0) {
				if (storageTanks.getYeastTank().getLitre() >= ((OpenTopContainer) container).getCapacity()) {
					...
					if ((((Factory) faci).getStorageTanks().getYeastTank().getLitre())
							+ temp.getLitre() <= ((Factory) faci).getStorageTanks().getYeastTank().getCapacity()) {
						result = transferControl(temp, faci, container);
					}
				}
	...
	/*
	 * Firstly, if container can take the given item from the storage tank(milkTank
	 * or creamTank or cacaoTank or yeastTank) then we control that storage tank can
	 * remove the given item ( it is always true actually ) from storage
	 * tank(milkTank or creamTank or cacaoTank or yeastTank). If these are true then
	 * we apply the transfer.
	 */
