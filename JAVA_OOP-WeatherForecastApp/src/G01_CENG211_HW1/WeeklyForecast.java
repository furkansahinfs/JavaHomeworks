...
public class WeeklyForecast {
	...
	// If the number of cityWeather is smaller than the capacity of first dimension,
	// we control that if the number of dates is smaller than capacity of second
	// dimension.
	// If # of dates is smaller we assign the cityWeather to the index. If not we
	// increase the number of cityWeather, make #ofdate zero and assign the
	// cityWeather to the index
	public void add(CityWeather cityWeather) {
		// I want to know what the first date of week is. ( like 3 - 14 - 15 etc ). So I
		// execute an if block which runs just once until program ends.
		if (dateFirst == false) {
		...
		if (numberOfCityWeather < sinir) {
			if (numberOfDate < sinir2) {
				ensureCapacity();
				cityWeatherList[cityWeather.getCity().getPlateNo() - 1][cityWeather.getDate().getDate()
						- dateX] = cityWeather;
				numberOfDate++;
			} else {
				numberOfCityWeather++;
