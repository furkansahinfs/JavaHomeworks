...
public class WeatherQuery {
	...
	// We take a cityName and control the visibility and wind of this city when we
	// wander in the cityWeatherList (weeklyForecast's list) and add the name to a
	// string.
		...
		for (int i = 0; i < numberOfCityWeather; i++) {
			for (int j = 0; j < 7; j++) {
				...
				if (CityName.equalsIgnoreCase(cityWeather.getCity().getName())) {
					if ((cityWeather.getWeather().getVisibility().equals("medium")
							|| cityWeather.getWeather().getVisibility().equals("high"))
							&& cityWeather.getWeather().getWind() < 40) {
	...
	public String getHighestAvgHumudity(ArrayX regionlist) {
		...
		// We control the regionName of the cityWeathers which are in the
		// cityWeatherList and regionList's members name. If names are equal we add the
		// humidity.
		// After we find the sum of all humidities in a region, we divide sum by number
		// of city list's members of the region.
		for (int k = 0; k < regionNumber; k++) {
			...
			// If regionOrtalama is greater than max, we keep the region's name and make max
			// equal to regionOrtalama.
	...
	public void GetFindestName() {
		...
		// We wander in the cityWeatherList (weeklyForecast's list) and find the average
		// temperature of cities.
		// After that we wander in the cityWeatherList and calculate the standard
		// deviation of cities
		// and add the result to the minArray list. Also add the City object to the
		// maxArray.
		// So we have two lists which behave as parallel lists, one of them holds
		// standard deviations of cities, other holds cities.
		for (int i = 0; i < numberOfCityWeather; i++) {
			for (int j = 0; j < 7; j++) {
				CityWeather cityWheater = (CityWeather) cityWeatherList[i][j];
		...
		// We want to be sure that there can exist same standart deviations of some
		// cities. So we try to find that how many cities have same standart deviations.
	...
	// We make minArray (holds standart deviations) sorted using bubbleSort, at the
	// same time we make maxArray (holds cities) sorted according simultaneously to
	// minArray.
	...
	// We control the weather of whole days of a city to find rainy days which are
	// tomorrow and next of tomorrow.
	public String Rainy() {
		String name = "";
		double prep;
		double prep2;
	...
	// We wander in the cityWeatherList (weeklyForecast's list) and find the lowest
	// feel like temp.
	// So we decribe a "x" to compare weathers.
	// If feel like temp of a weather is smaller than "x", we make x equals to
	// weather's feel like temp.
	// There can be more cities which have same feel like temp. So when we find
	// smaller temp, we clear the list and we add the city to the list.
	// If there exists cities which have same feel like temps, we add the cities to
	// the list without clear method.
	public ArrayX GetFlLowestName() {
		...
		for (int i = 0; i < numberOfCityWeather; i++) {
			for (int j = 0; j < 7; j++) {
				CityWeather cityWheater = (CityWeather) cityWeatherList[i][j];
				current = cityWheater.getWeather().getFltemperature();
	...
	// We wander in the cityWeatherList (weeklyForecast's list) and find the lowest
	// altitude and highest altitude cities.
	// and we add the weathers of the cities to list. Not cities, we add weathers of
	// those cities !
	public void MeanTemp() {
		...
		// After finding lowest and highest altitude of cities, we wander in the
		// minAltitudeList and maxAltitudeList and sum the temperature of weathers.
		// After that we calculate the mean temperatures and print them.
		double countMinAltArray = 0;
		double countMaxAltArray = 0;

		for (int i = 0; i < minAltArray.urunSayisi; i++) {
			countMinAltArray = countMinAltArray
					+ ((CityWeather) minAltArray.getList()[i]).getWeather().getTemperature();
		}
		for (int i = 0; i < maxAltArray.urunSayisi; i++) {
			countMaxAltArray = countMaxAltArray
					+ ((CityWeather) maxAltArray.getList()[i]).getWeather().getTemperature();
		}
		System.out.println(
				"4) " + countMaxAltArray / maxAltArray.urunSayisi + " , " + countMinAltArray / minAltArray.urunSayisi);

	}

	// We wander in the cityWeatherList (weeklyForecast's list) and find the lowest
	// temp.
	// So we describe a "x" to compare weathers.
	// If temp of a weather is smaller than "x", we make x equals to weather's temp.
	// There can be more cities which have same feel like temp. So when we find
	// smaller temp, we clear the list and we add the city to the list.
	// If there exists cities which have same temps, we add the cities to the list
