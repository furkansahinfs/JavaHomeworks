...
public abstract class Artwork implements IComparable ,ITradeable{
	...
	// If object is tradeable and buyer has enough money, we add the item to buyer's list and remove from seller's list. And money is changed.
	public void tradeToBuyer(Seller seller,Buyer buyer, double cost) {
		...
		if(this.isTradeable()) {
			if (buyer.getWallet()>= cost) {
				...
				seller.getList().remove(this); // not to sell one item to different buyer's, we remove the item temporarily.

			}
			else {
				// If there exists a situation which buyer cannot buy an artwork, we print a message.
				System.out.println("No money, no artwork for "+ buyer.getName());
			}
		}
		else
		{
			// Although there is no situation which buyer can buy a nontradeable item, we print a message for this situation.
