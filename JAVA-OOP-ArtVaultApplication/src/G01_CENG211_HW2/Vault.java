...
public class Vault {
	...
	private ArrayList<Object> givenList = new ArrayList<Object>(); // givenlist is returned from FileIO which holds the
																	// string of each line of csv file.

	public Vault(ArrayList<Object> givenList) {
		this.givenList = givenList;
	}

	// In for loop, we travel the list and split each line. According to object
	// number(1-2-3-4-5) we create the objects using their variables
	...
	public void split() {
		for (int i = 0; i < givenList.size(); i++) {
			...
			switch (objectNumber) {
			case 1:
				Painting painting = new Painting(strs[1], strs[2], strs[3], Double.parseDouble(strs[4]),
			...
			case 2:
				...
				break;
			case 3:
				ArrayList<String> architects = new ArrayList<String>(); // there can be architects more than one, so we
																		// holds them in a list.
				for (int j = 6; j < strs.length; j++) {
					architects.add(strs[j]);
				}
				Architecture architecture = new Architecture(strs[1], strs[2], Double.parseDouble(strs[3]),
						Double.parseDouble(strs[4]), Double.parseDouble(strs[5]), architects);
				...
				break;
			case 4:
				ArrayList<String> periods = new ArrayList<String>(); // there can be periods more than one, so we holds
																		// them in a list.
